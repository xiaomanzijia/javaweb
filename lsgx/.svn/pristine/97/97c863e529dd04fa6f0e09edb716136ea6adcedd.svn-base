...
<script type="text/javascript" src="../../js/jquery.imgareaselect.js"></script>
	...
	<div class="pageContent">
		<form onsubmit="return iframeCallback(this, img_u_dialogAjaxDone);"
			...
			action="action/uploadImgAjax.do" method="post">
			<div layouth="60" class="pageFormContent"
				style="height: 500px; overflow: auto;">
				<input type="hidden" value="${usertest.id}" name="id" />
				<div>
					<div align=center>
			...
			if (json.statusCode == '200') {
				...
				if ("closeCurrent" == json.callbackType) {
					if (json.navTabId) { //把指定navTab页面标记为需要“重新载入”。注意navTabId不能是当前navTab页面的
						...
						$.pdialog
								.reload(
								'/action/userTestEditGET.do?id=${usertest.id}',
								{
									dialogId : json.navTabId
								});
					}
					$.pdialog.close('usertest_upload_img');
				}
			}
		}
	</script>
